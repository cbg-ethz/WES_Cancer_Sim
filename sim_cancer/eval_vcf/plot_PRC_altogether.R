...
print(tools_PRC)
pdfToPlot <- paste(dirname(tools_PRC[[1]]),"/Precision_Recall_Curves_all.pdf",sep="")

myTitles <- c('deepSNV', 'GATK HP', 'GATK UG', 'JointSNVMix2', 'MuTect', 'SAMtools', 'SomaticSniper', 'VarScan2')

if(!file.exists(pdfToPlot)){
	mySize=20
	pdf(pdfToPlot,width=mySize,height=mySize*2, pointsize=8)
	...
	for(i in 1:8){
		...
		plot(recall[plot_IDX], precision[plot_IDX], pch=20,main=myTitles[i], xlab="Recall", ylab="Precision",ylim=c(0,1),xlim=c(0,1),cex.axis=5,cex.lab=5,cex.main=5)
	}
	dev.off()
}
