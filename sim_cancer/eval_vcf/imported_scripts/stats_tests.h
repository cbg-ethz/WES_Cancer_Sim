...
float binom_cdf(int cnt, int num, float p)
{
	if (cnt>=num)
		return 1.0; 
...
float binom_cdf_alternative(int cnt, int num, float p, string greater_or_less)
{
	if (cnt>=num)
		return 1.0;
	...
	if(greater_or_less.compare("less")==0){
		...
		printf("error: neither greater nor less\n");
		return -1.0;
	}
