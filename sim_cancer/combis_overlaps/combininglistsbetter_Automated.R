...
if(length(args)<4){
	cat(paste("Usage: combininglistsbetter_automated.R <outDir> <numTools(NotCountingMuTect),n={1,...,7}> <tool1> [<tool2> ... <tooln>]\n",sep=""))
	quit()
}
...
for(i in 3:length(args)){
	...
	if(grepl("muTect_SNVs_Raw.vcf",basename(currTool))){
		...
		muTectTool=currTool
		cat("We found muTect!\n")
		typeoftest <- paste(typeoftest,"muTect_",sep="")
	} else {
		cntTools=cntTools+1
		...
		if (grepl("gatk_SNVs",basename(currTool))) {
			 typeoftest <- paste(typeoftest,"gatk_SNVs_",sep="")
		} else if (grepl("gatkHPCaller",basename(currTool))) {
			typeoftest <- paste(typeoftest,"gatkHPCaller_",sep="")
		} else if (grepl("deepSNV",basename(currTool))) {
			typeoftest <- paste(typeoftest,"deepSNV_",sep="")
		} else if (grepl("jointSNVMix2",basename(currTool))) {
			typeoftest <- paste(typeoftest,"jointSNVMix2_",sep="")
		} else if (grepl("varscan2",basename(currTool))) {
			typeoftest <- paste(typeoftest,"varscan2_",sep="")
		} else if (grepl("somaticSniper",basename(currTool))) {
			typeoftest <- paste(typeoftest,"somaticSniper_",sep="")
		} else if (grepl("samvar",basename(currTool))) {
			typeoftest <- paste(typeoftest,"samvar_",sep="")
		} else {
...
if(ActualNumTools==8){ ## all tools - file name would become too large
	typeoftest <- "all_8_"
}
