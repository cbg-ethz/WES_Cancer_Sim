...
for fileIDX in `seq 1 ${#tools[@]}`; do
	...
	myPRcurveFileOrig=$path2PRorig/${beginnig}${tools[fileIDX-1]}${ending}
	if [[ "${tools[fileIDX-1]}" == "sinvict"  ]]; then
		...
		myPRcurveFileOrig=$path2PRorig/sinvict_TU.wCont20.final.RG.50perc_vs_NO_final.RG.50perc_somatic.vcf_indel0.PRcurve
	fi
