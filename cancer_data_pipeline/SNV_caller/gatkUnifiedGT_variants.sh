...
function gatkUnifiedGT_variants()
{
	...
	for fn_bam in $files ; do
		local outputTU=$out_dir/`basename $fn_bam | sed 's/.bam//'`.gatk_SNVs.raw.vcf
		local outputNO=$out_dir/`basename $fn_bam | sed 's/.bam//'`.NOsample.gatk_SNVs.raw.vcf
		local outputpaired=$out_dir/`basename $fn_bam | sed 's/.bam//'`.paired_TU_NO.gatk_SNVs.raw.vcf
		...
		if [ ! -f $outputTU ]; then
			...
			#command="$GATK -R $fn_genome -T UnifiedGenotyper -I $fn_bam -I $bam_norm -o $output --unsafe NO_READ_ORDER_VERIFICATION $rq_opt"
			command="$GATK -R $fn_genome -T UnifiedGenotyper -I $fn_bam -o $outputTU $rq_opt"
			echo $command
			eval $command
		else
		...
		if [ ! -f $outputNO ]; then
			...
			command="$GATK -R $fn_genome -T UnifiedGenotyper -I $bam_norm -o $outputNO $rq_opt"
			echo $command
			eval $command
			
		else
			echo "gatk output file already exists $outputNO."
		fi 	
		...
		if [ ! -f $outputpaired ]; then
			...
			# 255 to all reads with only a single alignment. With this option we modify this mapping quality
			rq_opt="-rf ReassignOneMappingQuality -RMQF 255 -RMQT 254"
			command="$GATK -R $fn_genome -T UnifiedGenotyper -I $fn_bam -I $bam_norm -o $outputpaired --unsafe NO_READ_ORDER_VERIFICATION $rq_opt"
			echo $command
			eval $command
		else
